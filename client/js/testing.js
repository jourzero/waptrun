...
$('#IURIs, #IEvidence, #IScreenshots, #INotes, c#PrjNotes, #TTesterSupport, #TIssueBackground, #TRemediationBackground').on('click', 
        function (event) {
    $("#" + event.target.id).height(200);
});

// When some fields are unselected, decrease the text box size to a default height
$('#IURIs, #IEvidence, #IScreenshots, #PrjNotes, #TTesterSupport, #TIssueBackground, #TRemediationBackground').on('blur', 
        function (event) {
...
function uiParseBurpIssue(){
    ...
    let issueName="", evidence="", urls="", newNotes="", sev=-1, sevText="", issueBG="", remedBG="";
